...
@export var webhook_bind_all_interfaces: bool = true
...
# Audio direkt hier verwalten
var audio_player: AudioStreamPlayer
var previous_level: int = 0  # Verfolgt das vorherige Level für Sound-Richtung

func _ready():
	print("🚀 Starte Ladder System...")
	
	# Audio Player als allererstes erstellen
	create_audio_player_immediately()
...
func create_audio_player_immediately():
	print("🔧 Erstelle Audio Player sofort...")
	audio_player = AudioStreamPlayer.new()
	audio_player.name = "EarlyAudioPlayer"
	add_child(audio_player)
	print("✅ Audio Player sofort erstellt: ", audio_player)
...
func transfer_exports_to_config():
	...
	config.webhook_bind_all_interfaces = webhook_bind_all_interfaces
...
func setup_audio():
	print("Audio-Setup wird gestartet...")
	print("   - Vor AudioStreamPlayer.new()")
	
	audio_player = AudioStreamPlayer.new()
	print("   - AudioStreamPlayer erstellt: ", audio_player)
	
	audio_player.name = "LadderAudioPlayer"
	audio_player.volume_db = config.volume_db
	print("   - Lautstärke gesetzt auf: ", config.volume_db, " dB")
	print("   - Eigenschaften gesetzt, füge zu Scene Tree hinzu...")
	
	add_child(audio_player)
	print("   - Zu Scene Tree hinzugefügt")
	print("AudioStreamPlayer Setup abgeschlossen")
...
func connect_signals():
	...
	webhook_manager.webhook_gift_received.connect(_on_webhook_gift)
...
func _on_level_changed(new_level: int):
	# Sound-Richtung anhand der Level-Änderung bestimmen
	print("🔊 Level geändert von ", previous_level + 1, "KMH zu ", new_level + 1, "KMH")
	
	# Falls Audio Player fehlt, jetzt erstellen
	if not audio_player:
		print("   ! Audio Player fehlt - erstelle jetzt!")
		audio_player = AudioStreamPlayer.new()
		audio_player.name = "LateAudioPlayer"
		add_child(audio_player)
		print("   ✅ Audio Player nachträglich erstellt: ", audio_player)
	
	# Sound basierend auf Level-Richtung abspielen
	if new_level > previous_level:  # Level ging hoch
		print("   -> Spiele UP Sound (", previous_level + 1, " -> ", new_level + 1, ")")
		play_up_sound()
	elif new_level < previous_level:  # Level ging runter
		print("   -> Spiele DOWN Sound (", previous_level + 1, " -> ", new_level + 1, ")")
		play_down_sound()
	
	# Vorheriges Level für nächsten Vergleich speichern
...
func play_up_sound():
	print("🎵 UP Sound angefordert...")
	if config.sound_up and audio_player:
		audio_player.stream = config.sound_up
		audio_player.play()
		print("   ✅ UP Sound wird abgespielt")
	else:
		print("   ❌ UP Sound kann nicht abgespielt werden")

func play_down_sound():
	print("🎵 DOWN Sound angefordert...")
	if config.sound_down and audio_player:
		audio_player.stream = config.sound_down
		audio_player.play()
		print("   ✅ DOWN Sound wird abgespielt")
	else:
		print("   ❌ DOWN Sound kann nicht abgespielt werden")
...
func _on_webhook_gift(level: int):
	print("📡 Webhook GIFT für Level " + str(level) + " empfangen")

# =============== GAME LOOP ===============
func _process(_delta):
	# Lautstärke live updaten falls im Inspector geändert
	if audio_player and config:
		if audio_player.volume_db != config.volume_db:
			audio_player.volume_db = config.volume_db
